...
test('admin can list tasks without user ownership filter', async () => {
  const task = {
    id: taskId,
  ...
  const app = await buildApp(db, 'admin')
  ...
  assert.equal(filteredColumns.includes('user_id'), false)
})
